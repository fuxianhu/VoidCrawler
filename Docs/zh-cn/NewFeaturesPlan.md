...
支持导入 C++ 代码文件来自动编译（内置 g++）和执行，如：
```
#include "VCCDev.h"
int main() {
    VCCDev::Plugin *p = new VCCDev::Plugin("My Plugin", "v.1.0.0", "<Author>YFY", "Description");
    VCCDev::AutoGUI::click(p, 114, 514, VCCDev::MouseKey::Right);
    delete p;
    return 0;
}
```
...
| 键 | 类型 | 默认值 | 描述 |
| --- | --- | --- | --- |
| 按键 `Key` | 单选 `Select` 或鼠标按键 `MouseKey` | 左键 `Left` | 需要点击的鼠标按键 |
| 双击 `DoubleClick` | 开关 `Bool` | 关 `False` | 需要点击的鼠标按键 |
