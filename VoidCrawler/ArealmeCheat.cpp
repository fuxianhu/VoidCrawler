...
bool ArealmeCheat::cheatWarning()
{
    QMessageBox::StandardButton result = QMessageBox::warning(
        ...
        R"(  ===== Warning 警告 =====
            1. 所有作弊行为可能会有 控制鼠标指针与键盘等输入设备（Auto Graphical User Interface 图形用户界面自动化）、恶意 DLL 注入进程、内存修改行为，客户端上的此类功能均为未经授权的第三方程序（外挂、修改器、作弊器等）。
...
int ArealmeCheat::testColorCheat()
{
    ...
    thread->start(); // 不调用 wait，让其在后台运行
	VCCore::logger->info("Arealme Color Test Cheat started in background thread.");
    return 0;
}
...
int ArealmeCheat::test1to50Cheat()
{
    ...
    thread->connect(thread, &QThread::finished, thread, &QThread::deleteLater);
    thread->start(); // 不调用 wait，让其在后台运行
    VCCore::logger->info("Arealme 1 to 50 Test Cheat started in background thread.");
    return 0;
}